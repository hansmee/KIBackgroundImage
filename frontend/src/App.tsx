...
export default function App() {
  ...
  const callApi = async () => {
    axios.get(`https://graph.instagram.com/me/media?access_token=${process.env.REACT_APP_INSTAGRAM_KEY}&fields=id,caption,media_type,media_url,thumbnail_url,permalink`).then((res)=>{
      console.log(res)
    })
  ...
  return (
    <div className="App">
      ...
      <div className="body">
        <div>
          <button className="kakao_login" disabled={kakaoLogin} onClick={showAgreement} title={kakaoLogin ? '이미 로그인되어있습니다.':''}>카카오톡 로그인하기</button>
          <button className="instagram_login"onClick={clickInsta}>인스타그램 로그인하기</button>
        </div>
      <button className="kakao_logout"onClick={handleLogout}>서비스에서 로그아웃하기</button>
